...
In particular functions to compute genus, $\mathbb{Q}$-Jacobian decomposition, number of $\mathbb{F}_q$-points with $p\nmid N$, 
and testing by counting $\mathbb{F}_{p^n}$-points if could be possible a degree $d$ map from $X_0(N)/W_N$ to an elliptic curve that appears in the $\mathbb{Q}$-decomposition of the quotient modular curve. The subgroup $W_N$ could be also the trivial subgroup or the full $B(N)$.
